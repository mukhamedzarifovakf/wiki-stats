...
class WikiGraph:
    ...
    def load_from_file(self, filename):
        ...
        with open(filename) as f:
            ...
            self._number_of_links_from = array.array('L', [0]*n)
            self._number_of_links_to = array.array('L', [0]*n)
            self._number_of_redirects_to = array.array('L', [0]*n)
            current = 0

            for i in range(n):
                self._titles.append(f.readline().rstrip())
                (size, redirect, number_of_links) = map(int, f.readline().split())
                self._sizes[i] = size
                self._redirect[i] = redirect
                self._number_of_links_from[i] = number_of_links
                self._offset[i+1] = self._offset[i] + number_of_links
                for j in range(current, current + number_of_links):
                    self._links[j] = int(f.readline())
                    self._number_of_links_to[self._links[j]] += 1
                    if self._redirect[i]:
                        self._number_of_redirects_to[self._links[j]] += 1
                current += number_of_links

        print('Граф загружен')

    def get_number_of_links_from(self, _id):
        number_from_id = self._number_of_links_from[_id]
        return number_from
    ...
    def analysis(self):
        ...
        print('Количество статей с перенаправлением: ', num_of_redirects)
        print('Минимальное количество ссылок из статьи: ', min_links_from)
